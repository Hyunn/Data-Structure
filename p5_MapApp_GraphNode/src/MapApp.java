...
public class MapApp {
	...
	public static NavigationGraph createNavigationGraphFromMapFile(String graphFilepath) throws InvalidFileException, FileNotFoundException{
		try {
			//file reader
			...
			//array of strings for reading first line
			String[] firstLine = line.split(" ");
			//handle invalid file
			if(firstLine.length < 3) 
				throw new InvalidFileException("Invalid File Exception!");
			//arrays of strings to store property names
			String[] edgePropertyNames = new String[firstLine.length-2];
			//run navigation graph
			NavigationGraph navigationGraph = 
					new NavigationGraph(edgePropertyNames);
			//getting the edge property names from the first line
			for(int i=0; i<firstLine.length-2; i++) {
				edgePropertyNames[i] = firstLine[i+2];
			}
			//read rest of file
			while ((line = br.readLine()) != null) {
				try {
					...
					Location src = new Location(locations[0]);
					Location dest = new Location(locations[1]);
					List<Double> temList = new ArrayList<Double>();
					//convert & store properties
					for(int i=2; i<locations.length; i++) {
						temList.add(Double.parseDouble(locations[i]));
					}
					//make new path to add to graph
					Path temPath = new Path(src, dest, temList);
					//store vertex and paths to graph
					navigationGraph.addVertex(src);
					navigationGraph.addVertex(dest);
					navigationGraph.addEdge(src, dest, temPath);
					
				}catch (Exception e) {
					System.out.println("Invaild line for " + line);
				}
			}
			...
			System.out.println("File cannnot found: " + graphFilepath);
		}catch(IOException e) {
			System.out.println("IO Exception for " + graphFilepath);
		}
		return null;
